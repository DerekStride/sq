...
pub fn build_cli() -> Command {
    ...
    let root_help = StyledStr::from(format!(
        "{header}Task file:{header:#}\n  By default, {literal}sq{literal:#} discovers the nearest existing {literal}.sift/issues.jsonl{literal:#} within the current git worktree and otherwise falls back to {literal}<cwd>/.sift/issues.jsonl{literal:#}\n  Override with {literal}-q, --queue <PATH>{literal:#} or {literal}SQ_QUEUE_PATH=<PATH>{literal:#}"
    ));
